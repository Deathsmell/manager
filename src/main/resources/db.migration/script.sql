...
alter table if exists cashbox
    add constraint cashbox_client foreign key (client_id) references client;

alter table if exists cashbox
    add constraint cashbox_master foreign key (master_id) references master;

alter table if exists client_masters
    add constraint FKt112pikhdepsvtgqo0svy910v foreign key (master_id) references master;

alter table if exists client_masters
    add constraint cm_clietn foreign key (client_id) references client;

alter table if exists user_roles
    add constraint ur_usr foreign key (user_id) references usr;